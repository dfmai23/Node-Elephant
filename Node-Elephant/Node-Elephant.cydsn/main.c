...
CY_ISR(isr_CAN_Handler){
    ...
    //int16 temp_throttle2 = temp_throttle;
    temp_throttle = (int32)(temp_throttle-throttle1Min)*0x7FFF / (throttle1Max-throttle1Min-((throttle1Max-throttle1Min)/16));
    //temp_throttle2 = (int32)(temp_throttle2-throttle2Min)*0x7FFF / (throttle2Max-throttle2Min-((throttle2Max-throttle2Min)/16));
    
    /*
    if (temp_throttle2 < temp_throttle)
        temp_throttle = temp_throttle2;
    */
    ...
    float32 temp = ADC_SAR_CountsTo_Volts(brake);
    if(ADC_SAR_CountsTo_Volts(brake) < .45)
    {
        force_stop = true;   
        temp_throttle = 0;
    }
